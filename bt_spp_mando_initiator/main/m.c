...
static const esp_spp_sec_t sec_mask = ESP_SPP_SEC_AUTHENTICATE; // ESP_SPP_SEC_AUTHORIZE; //ESP_SPP_SEC_AUTHENTICATE;
...
static const char uncord_baddr[] = "00:0b:ce:04:f1:cd";
  ...
  switch (event) {
    case ESP_BT_GAP_DISC_RES_EVT:
      ...
      for (int i = 0; i < param->disc_res.num_prop; i++) {
        ESP_LOGE(TAG, "remote address:[%s] %s", bda2str((uint8_t*)param->disc_res.bda, bda_str, sizeof(bda_str)), uncord_baddr);
        bda = (uint8_t*)param->disc_res.bda;
        sprintf(str, "%02x:%02x:%02x:%02x:%02x:%02x", bda[0], bda[1], bda[2], bda[3], bda[4], bda[5]);
        if (strcmp(str, uncord_baddr) == 0) {
